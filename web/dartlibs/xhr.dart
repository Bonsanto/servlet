...
class XHR{
  ...
  String  get response  => _xhr.responseText;
  String  get url       => _url;
  bool    get mode      => _mode;
  set method(String method)     => _method = method;
  set url(String url)           => _url = url;
  set mode(bool mode)           => _mode = mode;
  set callBackFunc(Function a)  => _extFunc = a;

  /*Constructor.*/
  XHR(String url, [String method = "POST", bool mode = true]){
    _xhr = new HttpRequest();
    _url = url;
    _method = method;
    _mode = mode ;

    _xhr.onReadyStateChange.listen((_){
      if(_xhr.readyState == 4 || _xhr.readyState == 0){
        if(_xhr.status == 200){
          _extFunc();
        }else{
          window.alert(errors[_xhr.status]);
        }
      }
    });
  }
  ...
  void send(String message){
    _xhr.open(_method, _url, async: _mode);
